...
class CreateProduct extends Component
{
    ...
    protected $rules = [
        ...
        'is_deal' => 'boolean',
        'old_price' => 'required_if:is_deal,true|numeric|min:0',
    ...
    public function updatedHasStorage()
    {
        ...
        // Don't automatically add storage options when enabled
        // User must manually click "Add Storage Option" button
    }
    ...
    private function uploadImages()
    {
        ...
        foreach ($this->product_images as $image) {
            // Use a simpler filename without subdirectory
            $filename = Str::random(40) . '.jpg';

            // Use the new images disk and store directly in products folder
            $path = Storage::disk('images')->putFileAs('products', $image, $filename);

            // Generate the public URL for the images folder
    ...
    {
        // Validate everything except product_images (already validated during upload)
        $this->validate([
            'product_name' => 'required|min:3',
            'price' => 'required|numeric|min:0',
            'category_id' => 'required|exists:categories,id',
        ...
        if (!empty($this->images_url)) {
            $manualUrls = array_filter(array_map('trim', explode("\n", $this->images_url)));
